---
title: "Figure4_correct_scale_heatmap_highly_correlated_TSS_and_survivals"
output:
  md_document:
...
```{r tpm_matrix,warning=FALSE,message=FALSE}
...
```{r gene37,warning=FALSE,message=FALSE}
...
```{r matrix_gene,warning=FALSE,message=FALSE}
matrix_mod_gene <- merge(matrix_mod,genes37,by="ensembl_gene_id")
matrix_mod_gene <- matrix_mod_gene %>% select(-ensembl_gene_id)
```

Upload the gene list of interest and use it to subset the original COMMPASS TPM dataframe.
```{r matrix_gene_sub,warning=FALSE,message=FALSE}
...
```{r clinical_info,warning=FALSE,message=FALSE}
...
```{r mut_info,warning=FALSE,message=FALSE}
...
Merge the clinical, survival and genetic information in a unique dataframe.
```{r clnical_and_mut}
...
Define patient groups based on the expression of the NRF1 gene and add this information to the dataframe created earlier.
```{r expr_nrf1_level}
...
Subset the original matrix to include only patients with clinical, survival, and mutational information.
```{r sub_matrix_clinical}
...
```{r col_anno,warning=FALSE,message=FALSE}
...
![Fig4d_heatmap_COMMPASS_highly_enriched_NRF1-regualted_signature](https://github.com/cleliacort/NRF1_paper/blob/main/Fig4/figures/heatmap_cluster_2_TSS_gene_highly_correlated_among_increase_disease_COMMPASS_only_tumour_with_NRF1_consensus_4cell_lines_0124_cluster_col_canberra_ward.D2_rows_manhattan_ward.D2_NRF1_expr_thr5_0124.png)


```{r save_heatmap, eval=FALSE,include=FALSE}
name_file=paste0("figures/heatmap_cluster_",num_col_split,"_",prefix,"_cluster_col_",distance_col,"_",method_col,"_rows_",distance_rows,"_",method_rows,"_NRF1_expr_thr5_0124.png",sep="")
ggexport(h,res=300,filename=name_file,width = 3000, height = 2000)
```

Store the cluster information to which each patient belongs.
```{r column_cluster_save,warning=FALSE,message=FALSE}
output_dir <- "figures/"
ht = draw(h)
...
Count the number of patients belonging to each disease stage for each cluster of the heatmap.
```{r iss_info_cluster_heatmap,warning=FALSE,message=FALSE}
...
Count the number of patients at each disease stage for each cluster in the heatmap.
```{r iss_cluster_info,warning=FALSE,message=FALSE}
...
Create a barplot.
```{r barplot_iss, eval=FALSE,include=FALSE}
...
```{r save_surv_plot, eval=FALSE,include=FALSE}
name_file_surv=paste0("figures/","survival_cluster_",num_col_split,"_",prefix,"_cluster_col_",distance_col,"_",method_col,"_rows_",distance_rows,"_",method_rows,"_1023.png",sep="")
ggexport(gg_surv,res=300,filename=name_file_surv,width = 3000, height = 2000)

```


```{r prova_iss_circular, warning=FALSE,message=FALSE}
...
```{r save_barplot_iss, eval=FALSE,include=FALSE}
name_file_barplot_iss=paste0("figures/","circularPlot_ISS_",cluster_sel,"_",num_col_split,"_",prefix,"_cluster_col_",distance_col,"_",method_col,"_rows_",distance_rows,"_",method_rows,"_1023.png",sep="")
name_file_barplot_iss=paste0("figures/","circularPlot_ISS_",cluster_sel,"_",num_col_split,"_",prefix,"_cluster_col_",distance_col,"_",method_col,"_rows_",distance_rows,"_",method_rows,"_1023_NO_LABELS.png",sep="")
ggexport(donut_chart,res=300,filename=name_file_barplot_iss,width = 1800, height = 1800)
```

![Fig4e_ISS_per_cluster](https://github.com/cleliacort/NRF1_paper/blob/main/Fig4/figures/circularPlot_ISS_cluster1_2_TSS_gene_highly_correlated_among_increase_disease_COMMPASS_only_tumour_with_NRF1_consensus_4cell_lines_0124_cluster_col_canberra_ward.D2_rows_manhattan_ward.D2_1023.png)

## Survival analysis based on patient subdivision from the heatmap 

Obtain and customize the survival information for the patient in the clusters of the heatmap.
```{r surv_analysis_prep}
...
```{r save_surv_plot_single, eval=FALSE,include=FALSE}
name_file_surv=paste0("figures/","survival_cluster_",num_col_split,"_",prefix,"_cluster_col_",distance_col,"_",method_col,"_rows_",distance_rows,"_",method_rows,"_1023.png",sep="")
ggexport(gg_surv,res=300,filename=name_file_surv,width = 3000, height = 2000)

```

![Fig4f_survival_by_NRF1_expression](https://github.com/cleliacort/NRF1_paper/blob/main/Fig4/figures/survival_cluster_2_TSS_gene_highly_correlated_among_increase_disease_COMMPASS_only_tumour_with_NRF1_consensus_4cell_lines_0124_cluster_col_canberra_ward.D2_rows_manhattan_ward.D2_1023.png)

## Survival analysis at each ISS based on patient subdivision from the heatmap

Conduct a survival analysis and create a plot based on the patient subdivisions derived from the heatmap for each disease stage.
```{r multi_surv_plot}
...
```{r save_multi_plot,eval=FALSE,include=FALSE}
# Store the plots in a unique figure
ggsave(paste0(output_dir,"/survival_COMMPASS_signature_high_corr_divided_per_ISS_0124.png"), re, width = 20, height = 8, dpi = 600)
```

![Suppl_Fig4_survival_by_iss](https://github.com/cleliacort/NRF1_paper/blob/main/Fig4/figures/survival_COMMPASS_signature_high_corr_divided_per_ISS_0124.png)
