# DESCRIPTION:
# This script processes gene expression data and visualizes correlations between selected target genes.
# It reads a list of genes (by gene name or Ensembl ID) and a matrix of normalized read counts. 
# The script allows users to:
# 1. Subset the data to include only the target genes.
# 2. Generate a heatmap showing the Pearson correlation of gene expression across samples.
# 3. Cluster genes and samples based on specified distance metrics and clustering methods.
# 4. Optionally save the row cluster memberships for further analysis.
# 
# INPUT:
# - A normalized reads count matrix.
# - A file containing a list of genes of interest.
# - User-specified parameters for clustering, plotting, and saving outputs.
# 
# OUTPUT:
# - A heatmap of the Pearson correlation matrix.
# - Optionally, cluster membership files for rows (genes).
# - Subsetted matrices based on the target genes and correlation matrix files.
#--------------------
# Upload packages
...
cat("\n")
# color_palette <- rev(brewer.pal(n = 6, name = "RdYlBu"))
# breaks <- c( 0,0.2,0.4,0.6,0.8, 1)
# color_ramp_function <- colorRamp2(breaks, color_palette)
# color_ramp_function <- c("#CB997E","#DDBEA9","#FFE8D6","#B7B7A4","#A5A58D","#6B705C")
...
h <- Heatmap(correlation_matrix,
             #column_title = "Gene INCRESING with disease progression COMMPASS",
...
# svg(complete_name_plot_svg, width = 12, height = 9)
# print(h)
# dev.off()

# ggsave(complete_name_plot_svg, plot = h,units = c("cm"),width=30, height=20)
